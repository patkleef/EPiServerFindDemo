...
var didyoumeanViewModel = function () {
    var self = this;
    ...
    function search(query) {
        if (self.useJavascript()) {
            $.get('/find_v2/_didyoumean?query=' + query + '&size=5', function(data) {
                self.resultJson(JSON.stringify(data, null, 4));
                self.results(data.hits);
            });
        }
        else {
            $.get('/api/Search/DidYouMean?query=' + query, function (data) {
...
ko.applyBindings(new didyoumeanViewModel(), document.getElementById("statistics-didyoumean"));
